...
3. **校验节点格式（强制）** - 执行校验命令：
   ```bash
   python3 .codebuddy/skills/testbuddy-skill/scripts/validate_nodes.py <test_cases.md路径>
   ```
...
4. **解析并替换 PARENT_UID 中文引用（强制）** - 脚本 `parse_structured_markdown` 不会自动将 Markdown 中的中文 PARENT_UID 引用（如 `模块1`、`测试场景1.1`）转换为实际的节点 uid。**必须在添加节点前执行以下步骤**：
   ...
   ```python
   import json, sys, re
   sys.path.insert(0, '.codebuddy/skills/testbuddy-skill/scripts')
...
6. **添加节点到脑图** - 使用 `add_nodes` 能力执行添加（**注意使用步骤4生成的 JSON 文件**）：
   ```bash
   python3 .codebuddy/skills/testbuddy-skill/scripts/write_node_from_file.py add <design_uid> /tmp/test_cases_resolved.json
   ```
