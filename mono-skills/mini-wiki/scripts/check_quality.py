...
class QualityMetrics:
    ...
    has_unreplaced_templates: bool = False  # 是否有未替换的模板变量
    mermaid_issues: int = 0  # Mermaid 语法问题数
...
def check_template_completeness(content: str) -> List[str]:
    ...
    critical_vars = ['PROJECT_NAME', 'MODULE_NAME', 'TECH_STACK', 'VERSION']

    for placeholder in placeholders:
        if placeholder in critical_vars:
            issues.append(f"关键模板变量未替换: {{{{ {placeholder} }}}}")
        else:
            issues.append(f"模板变量未替换: {{{{ {placeholder} }}}}")
...
def validate_mermaid_blocks(content: str) -> List[str]:
    ...
    for i, block in enumerate(mermaid_blocks, 1):
        ...
        if not has_type:
            issues.append(f"Mermaid 块 #{i} 缺少类型声明（如 flowchart, sequenceDiagram 等）")

        # 检查中文文本是否包裹在双引号中
        # 匹配含中文但未被双引号包裹的节点文本
        for line_num, line in enumerate(block_stripped.split('\n'), 1):
            # 跳过注释行和类型声明行
            if line.strip().startswith('%%') or line.strip() in valid_types:
                continue
            # 检测包含中文字符但未被双引号包裹的内容
            # 匹配方括号 [] 或圆括号 () 中的中文（这些在 flowchart 中需要引号）
            chinese_in_brackets = re.findall(r'[\[\(]([\u4e00-\u9fff][^"\]\)]*[\]\)])', line)
            for match in chinese_in_brackets:
                if '"' not in match:
                    issues.append(
                        f"Mermaid 块 #{i} 第 {line_num} 行: 中文文本建议用双引号包裹"
                    )
                    break  # 每行只报告一次
...
def count_tables(content: str) -> int:
    ...
    for line in content.split('\n'):
        ...
        # 检测表格行：匹配 |...|...|  模式，但排除分隔行
        is_table_row = bool(re.match(r'^\|.*\|.*\|', line_stripped))
        is_separator = bool(re.match(r'^\|[\s\-:|]+\|$', line_stripped))

        if is_table_row and not is_separator:
            if not in_table:
                # 新表格开始（第一行是表头，不计入数据行，但标记表格存在）
                in_table = True
                table_count += 1
...
def analyze_document(file_path: str) -> QualityMetrics:
    ...
    # 检查源码追溯（支持相对路径和 file:// 两种格式）
    metrics.has_source_tracing = bool(
        re.search(
            r'\*\*源码引用\*\*|\*\*Section sources\*\*|\*\*Diagram sources\*\*'
            r'|file://'
            r'|\[`[^`]+`\s+L\d+.*?\]\(\.\./'
            r'|\[📄\]\(\.\./',
            content
    ...
    metrics.has_unreplaced_templates = len(template_issues) > 0
    metrics.issues.extend(template_issues)

    # 验证 Mermaid 语法
    mermaid_issues = validate_mermaid_blocks(content)
    metrics.mermaid_issues = len(mermaid_issues)
    metrics.issues.extend(mermaid_issues)

    # 评估质量等级
    metrics.quality_level = evaluate_quality_level(metrics)

    # 生成问题列表（追加到已有的模板和 mermaid 问题之后）
    metrics.issues.extend(generate_issues(metrics))
...
def calculate_dynamic_expectations(source_path: str, doc_path: str) -> Dict:
    """基于源码分析动态计算质量期望值"""
    result = {
        "source_lines": 0,
        "source_files": 0,
        "exports": 0,
        "expected_min_lines": 150,
        "expected_min_diagrams": 1,
        "expected_min_examples": 2,
        "expected_min_sections": 6,
    }
        ...
        for src_file in source_dir.rglob(ext):
            ...
            try:
                with open(src_file, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    # 统计导出（支持多种语言）
                    total_exports += len(re.findall(
                        r'^export\s|^module\.exports|^pub\s+fn|^pub\s+struct|^public\s+class|^def\s+\w+|^class\s+\w+',
                        file_content, re.MULTILINE
                    ))
            except Exception:
                pass

    result["source_lines"] = total_lines
    result["source_files"] = total_files
    result["exports"] = total_exports

    # 动态公式计算期望值
    result["expected_min_lines"] = max(150, int(total_lines * 0.3 + total_exports * 15))
    # 动态公式与 SKILL.md 和 quality-standards.md 对齐
    import math
    result["expected_min_diagrams"] = max(1, math.ceil(total_files / 5))
    result["expected_min_examples"] = max(2, int(total_exports * 0.5))
    # 章节数 = 6 + 模块角色权重（core +4, util +2, config +1）
    # 此处无法精确判断角色，取默认 +2
    result["expected_min_sections"] = 6 + 2

    return result


def print_dynamic_analysis(source_path: str, wiki_path: str):
    """打印动态复杂度分析结果"""
    print("\n" + "=" * 60)
    print("🔬 动态复杂度分析")
    print("=" * 60)
    ...
    if not wiki_dir.exists():
        print("⚠️ Wiki 目录不存在，仅显示源码分析结果\n")

    expectations = calculate_dynamic_expectations(source_path, wiki_path)

    print("### 源码统计\n")
    print(f"| 指标 | 数值 |")
    print(f"|------|------|")
    print(f"| 源码行数 | {expectations['source_lines']} |")
    print(f"| 源码文件数 | {expectations['source_files']} |")
    print(f"| 导出/公开接口数 | {expectations['exports']} |")
    print()

    print("### 动态期望值 vs 实际值\n")
    print(f"| 指标 | 动态期望 | 说明 |")
    print(f"|------|----------|------|")
    print(f"| 文档最少行数 | {expectations['expected_min_lines']} | max(150, 源码行数×0.3 + 导出数×15) |")
    print(f"| 最少图表数 | {expectations['expected_min_diagrams']} | max(1, ceil(文件数÷5)) |")
    print(f"| 最少代码示例 | {expectations['expected_min_examples']} | max(2, 导出数×0.5) |")
    print(f"| 最少章节数 | {expectations['expected_min_sections']} | 6 + 模块角色权重 |")
    print()

    # 如果有 wiki 文档，显示对比
    if wiki_dir.exists():
        print("### 文档达标情况\n")
        print(f"| 文档 | 行数 | 期望行数 | 图表 | 期望图表 | 达标 |")
        print(f"|------|------|----------|------|----------|------|")
        for md_file in wiki_dir.rglob("*.md"):
            metrics = analyze_document(str(md_file))
            rel_path = os.path.basename(str(md_file))
            lines_ok = "✅" if metrics.line_count >= expectations['expected_min_lines'] else "❌"
            diagrams_ok = "✅" if metrics.diagram_count >= expectations['expected_min_diagrams'] else "❌"
            status = "✅" if lines_ok == "✅" and diagrams_ok == "✅" else "❌"
            print(
                f"| {rel_path} | {metrics.line_count} | {expectations['expected_min_lines']} "
                f"| {metrics.diagram_count} | {expectations['expected_min_diagrams']} | {status} |"
            )
        print()
...
def print_report(report: QualityReport, verbose: bool = False):
    ...
    # 模板和 Mermaid 问题汇总
    template_docs = [d for d in report.docs if d.has_unreplaced_templates]
    mermaid_docs = [d for d in report.docs if d.mermaid_issues > 0]
    if template_docs:
        print(f"- {len(template_docs)} 个文档存在未替换的模板变量")
    if mermaid_docs:
        print(f"- {len(mermaid_docs)} 个文档存在 Mermaid 语法问题")
    ...
    for doc in report.docs:
        data["docs"].append({
            ...
            "metrics": {
                ...
                "has_unreplaced_templates": doc.has_unreplaced_templates,
                "mermaid_issues": doc.mermaid_issues
            },
...
示例:
  ...
  python check_quality.py . --analyze-complexity /path/to/source
        """
    )
    parser.add_argument(
        "wiki_path",
    ...
    if args.analyze_complexity:
        source_path = args.analyze_complexity
        if not os.path.exists(source_path):
            print(f"❌ 源码路径不存在: {source_path}")
        else:
            print_dynamic_analysis(source_path, wiki_path)
